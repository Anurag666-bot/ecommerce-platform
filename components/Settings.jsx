...
const Settings = () => {
  ...
  // CORRECT: fetchUserData is defined at component scope
  ...
  // CORRECT: useEffect calls the function properly
  useEffect(() => {
    fetchUserData();
  }, [session]); // Added session as dependency

  const handlePasswordChange = (e) => {
    setPassword({ ...password, [e.target.name]: e.target.value }); // Fixed typo: e.targer -> e.target
  };

  const handleCommunicationPrefsChange = (key) => { // Added parameter
  ...
  const deleteAccount = async () => {
    if (
      window.confirm(
        "Are you sure you want to DELETE your account? This is not recoverable!"
      )
    ) {
      setIsLoading(true);
      try {
                ...
                {!showPasswordForm ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <p className="text-xs">
                      ...
                      external provider you can't change the password!
                    </p>
                  </div>
                )}
              </div>
